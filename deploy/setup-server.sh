...
# PHP hardening (no shell functions, no URL fopen, no uploads, strict/secure
# session cookies) — a conf.d drop-in, so it survives PHP package upgrades.
PHP_VER="$(php -r 'echo PHP_MAJOR_VERSION.".".PHP_MINOR_VERSION;')"
install -m 644 "$APP_DIR/deploy/php-hardening.ini" "/etc/php/${PHP_VER}/apache2/conf.d/99-officecardprint.ini"
